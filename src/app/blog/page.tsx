            ...
            {data && (
                <div className="mt-16 grid grid-cols-4 w-full gap-2 p-5">
                    {data.findBlogPosts.map((post: any) => (
                        <div
                            key={post.id}
                            className="bg-gray-900 p-5 rounded-lg shadow-lg border-l-4 border-[#cc00ff] hover:border-gray-900 hover:bg-[#cc00ff] cursor-pointer group transition-all duration-200"
                        >
                            <span className="text-sm text-[#cc00ff] group-hover:text-black">
                                {getElapsedTime(post.createdAt)}
                            </span>
                            {post.title && (
                                <h1 className="text-4xl text-md mb-5 uppercase font-bold text-[#cc00ff] group-hover:text-black">
                                    {post.title}
                                </h1>
                            )}
                            {post.description && (
